...
test_util.with_cleanup(restore_modules, function()
	do
		...
		assert_true(command:match("elif %[% %-d /home/root %]" ) ~= nil, "start_record should fallback to /home/root")
		assert_true(command:match('mapfile %-t topics < <%(ros2 topic list | rg \\"%^/livox\\"%)') ~= nil, "start_record should collect /livox topics")
