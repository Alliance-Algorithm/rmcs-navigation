...
local function configure_test_rule()
	...
	rule.road_zone_final.ours = { x = 0.0, y = 0.0 }
	rule.fortress.ours = { x = 0.0, y = 0.0 }
	rule.central_highland_near_crossing_road.ours = { x = 0.0, y = 0.0 }
	rule.central_highland_near_doghole.ours = { x = 0.0, y = 0.0 }

	-- Them side sample points
	rule.resupply_zone.them = { x = 0.0, y = 0.0 }
	rule.road_zone_begin.them = { x = 0.0, y = 0.0 }
	rule.road_zone_final.them = { x = 0.0, y = 0.0 }
	rule.fortress.them = { x = 0.0, y = 0.0 }
	rule.central_highland_near_crossing_road.them = { x = 0.0, y = 0.0 }
			...
			if condition.low_health() or condition.low_bullet() then
				...
				if job.done then
					if job.success then
						...
						action:warn("fsm(start_cruise): 导航失败")
					end
				end
			end,
		})
		...
		event = function(handle)
			if condition.low_health() or condition.low_bullet() then
				...
				if job.done and not job.success then
					action:warn("fsm(keep_cruise): 导航失败")
				end
			end,
		})
