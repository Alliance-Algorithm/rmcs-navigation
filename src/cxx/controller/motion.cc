...
struct MotionFsm::Impl
{
  ...
  LowPassFilter<Eigen::Vector2d> road_filter{ Tau<1.0>{},
                                              Eigen::Vector2d::Zero() };
  ...
  LowPassFilter<Eigen::Vector2d> slope_filter{ Tau<3.0>{},
                                               Eigen::Vector2d::Zero() };
  ...
  {
    return Eigen::Vector2d{ context.x, context.y };
  }
  ...
  auto update_gimbal_target() -> Eigen::Vector2d
  {
    ...
    if (std::isfinite(wy) && std::isfinite(ly)) {
      ...
      if (!yaw_bias_initialized && std::isfinite(wy)) {
        ...
        yaw_bias =
          normalize_yaw(yaw_bias + kYawBiasCorrectionGain * bias_error);
        last_world_yaw = wy;
      }
    }
    ...
    , wrap{ node }
  {
    ...
    /// 普通模式
    fsm.use<Status::NORMAL>([this] { wrap.info("MotionFsm::Enter | NORMAL"); },
                            [this] {
                              command.chassis_mode = ChassisMode::ALIGNMENT;
                              command.chassis_speed =
                                context.target_chassis_speed;
    ...
    /// 战斗模式
    fsm.use<Status::ATTACK>([this] { wrap.info("MotionFsm::Enter | ATTACK"); },
                            [this] {
                              command.chassis_mode = context.under_attack
                                                       ? ChassisMode::SPIN_FAST
                                                       : ChassisMode::SPIN_SLOW;
                              command.chassis_speed =
                                context.target_chassis_speed;
                              command.gimbal_toward = update_gimbal_target();
    ...
    /// 下台阶模式
    fsm.use<Status::STEP>([this] { wrap.info("MotionFsm::Enter | STEP"); },
                          [this] {
    ...
    fsm.use<Status::SLOPE>(
      ...
      [this] {
        ...
        command.chassis_speed =
          slope_filter.update(context.target_chassis_speed);
        command.gimbal_toward = update_gimbal_target();
        return Status::SLOPE;
      });
...
auto
MotionFsm::switch_mode(const std::string& mode) -> void
{
  pimpl->switch_mode(mode);
}

auto
MotionFsm::spin_once() noexcept -> Command
{
  return pimpl->spin_once();
}
